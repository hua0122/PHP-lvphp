...
class UserController extends AdminbaseController{
    ...
    //添加至土豪组
    public function addToWealthyGroup() {
        $option = D('Common/Options');
        //获取openId
        $openId = I('post.openid');
        empty(C('WEALTHY')) ? $wealthyGroup = [] : $wealthyGroup = C('WEALTHY');
        if (in_array($openId, $wealthyGroup)) {
            $this -> ajaxReturn(['msg' => '该用户已是VIP']);
        }
        $wealthyGroup[] = $openId;
        $config = [];
        $config['WEALTHY'] = $wealthyGroup;
        ...
        //将土豪组数据入库
        $data = [];
        $data['option_name'] = "wealthy_group_member";
        $data['option_value'] = json_encode($wealthyGroup);
        if ($option -> where("option_name='wealthy_group_member'") -> find()) {
            $result = $option -> where("option_name='wealthy_group_member'") -> save($data);
        }
        else {
            $result = $option -> add($data);
        }
        if ($result !== false) {
            $this -> ajaxReturn(['msg' => '添加VIP成功']);
        }
        else {
            $this -> ajaxReturn(['msg' => '添加VIP失败']);
        }
    }

    //从土豪组中移除
    public function removeFromWealthyGroup() {
        $option = D('Common/Options');
        //获取openId
        $openId = I('post.openid');
        ...
        //不存在土豪组配置时
        if (empty(C('WEALTHY'))){
            $this -> ajaxReturn(['msg' => '该用户不是VIP']);
        }
        $wealthyGroup = C('WEALTHY');
        if (!in_array($openId, $wealthyGroup)) {
            $this -> ajaxReturn(['msg' => '该用户不是VIP']);
        }
        //删除指定openId
        $openIdIndex = array_search($openId, $wealthyGroup);
        unset($wealthyGroup[$openIdIndex]);
        $wealthyGroup = array_values($wealthyGroup);
        ...
        if ( $result !== false ) {
            $this -> ajaxReturn(['msg' => '移除VIP成功']);
        }
        else {
            $this -> ajaxReturn(['msg' => '移除VIP失败']);
        }
    }
