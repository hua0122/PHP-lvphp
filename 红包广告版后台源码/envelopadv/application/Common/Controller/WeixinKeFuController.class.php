...
class WeixinKeFuController extends Controller {
	...
	private $token = 'fHYSM9RaNJA4e774';
	// 方法静态化
	...
	public function responseMsg() {
		...
		if (! empty ( $postStr ) && is_string ( $postStr )) {
			...
			if (! empty ( $postArr ['MsgType'] ) && $postArr ['MsgType'] == 'text') { // 文本消息
				...
				$linkCont = S('linkCont');
				if (empty($linkCont)) {
					$customerOptions = M('options_model')->where("option_name='customer_options'")->find();
					$linkCont= json_decode($customerOptions, true);
					S('linkCont', $linkCont);
				}
				$content = '您好，有什么能帮助你?';
				$data = array (
